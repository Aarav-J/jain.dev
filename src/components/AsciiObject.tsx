...
function sniffKind(bytes: Uint8Array): AssetKind | null {
  ...
  let head = "";
  try { head = new TextDecoder().decode(bytes.subarray(0, 2048)).replace(/^\uFEFF/, "").trimStart(); }
  catch { return null; }
...
function traceContours(inside: Uint8Array, width: number, height: number) {
  ...
  for (let y = 0; y < height - 1; y++) {
    for (let x = 0; x < width - 1; x++) {
      ...
      const code =
        inside[base] | (inside[base + 1] << 1) |
        (inside[base + width + 1] << 2) | (inside[base + width] << 3);
      if (code === 0 || code === 15) continue;
      const top = x + 0.5, right = y + 0.5;
      switch (code) {
  ...
  for (let start = 0; start < count; start++) {
    ...
    const points: number[] = [];
    let current = start;
    let x = segments[start * 4], y = segments[start * 4 + 1];
    while (current >= 0 && !used[current]) {
      ...
      let next = -1;
      if (bucket) for (const candidate of bucket) if (!used[candidate]) { next = candidate; break; }
      current = next;
    }
...
function simplify(points: number[], tolerance: number) {
  ...
  while (stack.length) {
    ...
    const dx = points[last * 2] - ax, dy = points[last * 2 + 1] - ay;
    ...
    for (let i = first + 1; i < last; i++) {
      ...
      const ox = px - dx * clamped, oy = py - dy * clamped;
  ...
  const result: number[] = [];
  for (let i = 0; i < count; i++) if (keep[i]) result.push(points[i * 2], points[i * 2 + 1]);
  return result;
}
...
function buildShapes(canvas: HTMLCanvasElement, aspectW: number, aspectH: number) {
  ...
  const traceW = trace.width, traceH = trace.height;
  const data = ctx.getImageData(0, 0, traceW, traceH).data;
  const width = traceW + 2, height = traceH + 2;
  ...
  const rings = traceContours(inside, width, height)
    .map((points) => simplify(points, SIMPLIFY_TOL))
...
function createImageObject(canvas: HTMLCanvasElement, anisotropy: number): THREE.Mesh {
  ...
  const aspectW = canvas.width / longest, aspectH = canvas.height / longest;
  const geometry = new THREE.ExtrudeGeometry(buildShapes(canvas, aspectW, aspectH), {
    depth: EXTRUDE_DEPTH, bevelEnabled: true,
    bevelThickness: BEVEL_SIZE, bevelSize: BEVEL_SIZE,
    bevelOffset: 0, bevelSegments: 2, steps: 1, curveSegments: 1,
  });
...
export function createAsciiObject(
  ...
  function buildRoom() {
    ...
    roomScene.add(room);
    for (const [x, z] of [[-15,15],[15,15],[15,-15],[-15,-15]] as [number,number][]) {
  ...
  const onMotionChange = () => { reducedMotion = motionQuery.matches; if (reducedMotion) floatGroup.rotation.set(0, 0, 0); applyOptions(); };
  ...
  function rebuildAtlas() {
    ...
    const padW = cellW + ATLAS_PAD * 2, padH = cellH + ATLAS_PAD * 2;
    ...
    (postMaterial.uniforms.uAtlasPad.value as THREE.Vector2).set(ATLAS_PAD / padW, ATLAS_PAD / padH);
    (postMaterial.uniforms.uAtlasInner.value as THREE.Vector2).set(cellW / padW, cellH / padH);
  ...
  function applyOptions() {
    ...
    (postMaterial.uniforms.uColor.value as THREE.Color).setStyle(config.color || "#ffffff", THREE.NoColorSpace);
    postMaterial.uniforms.uHasBg.value = config.background ? 1 : 0;
    if (config.background)
      (postMaterial.uniforms.uBackground.value as THREE.Color).setStyle(config.background, THREE.NoColorSpace);
    rebuildAtlas();
    syncCellGrid();
    applyRoughness();
    applyFit();
  ...
  function resize() {
    ...
    const deviceW = Math.round(width * pr), deviceH = Math.round(height * pr);
  ...
  function tick(time: number) {
    ...
    if (config.ascii) { renderer.setRenderTarget(cellTarget); renderer.render(cellScene, postCamera); }
    renderer.setRenderTarget(null);
    renderer.render(postScene, postCamera);
  }

  function startLoop() { if (loopRunning || !inView || disposed) return; loopRunning = true; renderer.setAnimationLoop(tick); }
  function stopLoop() { if (!loopRunning) return; loopRunning = false; renderer.setAnimationLoop(null); }
  ...
  return {
    setOptions(next: AsciiObjectOptions) {
      ...
      for (const [key, value] of Object.entries(next)) {
        ...
        if (config[key as keyof AsciiObjectOptions] !== value) { changed = true; break; }
      }
